...
__int64 WINAPI MyRtlUserThreadStart(PTHREAD_START_ROUTINE pfnStartAddr, PVOID pvParam)
{
	if ((HANDLE)pfnStartAddr >= global::g_hInst && (HANDLE)pfnStartAddr <= (global::g_hInst + global::g_dwModuleSize))
	{
		...
		static _BaseThreadInitThunk BaseThreadInitThunk = (_BaseThreadInitThunk)GetProcAddress(GetModuleHandleA("kernel32.dll"), __("BaseThreadInitThunk"));
...
void StartHook()
{
	...
	if (hNtDll != NULL)
	{
		_NtQueryVirtualMemory ZwQueryVirtualMemory = (_NtQueryVirtualMemory)GetProcAddress(hNtDll, __("NtQueryVirtualMemory"));
		_ZwReadVirtualMemory ZwReadVirtualMemory = (_ZwReadVirtualMemory)GetProcAddress(hNtDll, __("ZwReadVirtualMemory"));
		_ZwProtectVirtualMemory ZwProtectVirtualMemory = (_ZwProtectVirtualMemory)GetProcAddress(hNtDll, __("ZwProtectVirtualMemory"));
		...
		if ((sts = NTMakeHook::NTEnableHook(ZwQueryVirtualMemory)) != MH_OK)
		{
			// DbgPrintA("ZwQueryVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = NTMakeHook::NTEnableHook(ZwReadVirtualMemory)) != MH_OK)
		{
			// DbgPrintA("ZwReadVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = NTMakeHook::NTEnableHook(ZwProtectVirtualMemory)) != MH_OK)
		{
			// DbgPrintA("ZwProtectVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = NTMakeHook::NTEnableHook(RtlGetNativeSystemInformation)) != MH_OK)
		{
			// DbgPrintA("RtlGetNativeSystemInformation 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = NTMakeHook::NTEnableHook(&IsBadReadPtr)) != MH_OK)
		{
			// DbgPrintA("IsBadReadPtr 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = NTMakeHook::NTEnableHook(VirtualQuery)) != MH_OK)
		{
			// DbgPrintA("VirtualQuery 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = NTMakeHook::NTEnableHook(ZwQueryInformationThread)) != MH_OK)
		{
		...
		if ((sts = NTMakeHook::NTEnableHook(EnumProcessModulesEx)) != MH_OK)
		{
